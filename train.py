...
def get_or_build_tokenizer(config, ds, lang) -> Tokenizer:
    ...
    if not Path.exists(tokenizer_path):
        ...
        trainer = WordLevelTrainer(special_tokens = ['[UNK]', '[PAD]', '[SOS]', '[EOS]'], min_frequensy=2)
...
def get_ds(config):
    ...
    val_ds = BilingualDataset(validation_ds_raw, tokenizer_src, tokenizer_tgt, config['lang_src'], config['lang_tgt'], config['seq_len'])
...
def train_model(config):
    ...
    if config['preload']:
        ...
        optimizer.load_state_dict(state['oprimizer_state_dict'])
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            # Run the tensors through the model transformer
